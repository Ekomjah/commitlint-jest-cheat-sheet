...
## A brief walkthrough on how to set up React Testing Library in Vitest when using Vite. The tutorial assumes that you have already created a React project with Vite, either in JavaScript or TypeScript. 

### Next, install Vitest on the command line:
...
```
You can see that Vitest comes with test suites (here: describe), test cases (here: it) and assertions (here: expect().toBe()). If you have been using Jest before, you should be familiar with it, because Vitest acts as a replacement for it.
...
```
Second, include it in the Vite configuration file:
...
describe('App', () => {
  it('renders headline', () => {
    ...
    // check if App components render headline
  });
});
```
